...
* **Encoder-decoder adapter:** configure `TransformersEncoderDecoderSymbolGenerator` with an explicit model ID and full lowercase 40-character model commit digest. It records that exact digest, loads `.[neural]` lazily, constrains decoding to the current document's symbols plus EOS, permits only padding after EOS, and post-validates the decoded plan. No trained checkpoint, training result, or benchmark is included.
