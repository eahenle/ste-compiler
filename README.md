...
* **Training data:** use `plan-symbols --json` to export canonical `(serialized IR, symbolic plan)` records. Current plans start with `PLAN_EXACT_WHITESPACE_V1`, percent-encode terminology IDs, and otherwise contain only `WORD_*`, `TERM_*`, `UNIT_*`, punctuation, `SPACE`/opaque whitespace, newline, and document-specific number symbols. Markerless legacy plans retain conventional implicit spacing.
